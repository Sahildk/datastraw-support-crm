...
export default function SearchFilter({ search, setSearch, statusFilter, setStatusFilter, onNewTicketClick }) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between p-4 bg-[#121214] border border-zinc-800/80 rounded-2xl mb-6 shadow-md shadow-black/10">
      {/* Search Input */}
      ...
      <div className="flex flex-col sm:flex-row gap-3.5 items-stretch sm:items-center">
        ...
        <div className="flex items-center gap-1 p-1 bg-[#09090b] border border-zinc-800/80 rounded-xl overflow-x-auto custom-scrollbar">
          {statuses.map((status) => (
            <button
              key={status.value}
              onClick={() => setStatusFilter(status.value)}
              className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all duration-300 shrink-0 ${
